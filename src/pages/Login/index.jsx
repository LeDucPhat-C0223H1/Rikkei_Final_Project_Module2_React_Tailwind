...
const Login = () => {
  ...
  const { userInfor, isError, message } = useSelector(
  ...
  useEffect(() => {
    // if (userInfor == null) {
    //   if (token) {
    //     dispatch(checkTokenLocal(token));
    //   }
    // } else {
    //   navigate("/");
    // }
  ...
  return (
    <LayoutRegisLogin name="Đăng nhập">
      ...
      >
        <div className="bg-white rounded-md  w-[550px] h-[450px] ">
          ...
          <div className="flex justify-center">
            <div>
              ...
              <div className="flex justify-center relative mt-5">
                <button
                  type="submit"
                  className=" uppercase w-[400px] h-10 mt-5 bg-gradient-to-r from-cyan-500 to-blue-500 text-white rounded-md"
                >
                  Đăng Nhập
                </button>
              </div>
     ...
     {isSuccess && (
        <Notification show={isSuccess} message={registerMessage} type={'success'} handleDestroyErr={handleRegisterDestroyErr} />
      )}
    </LayoutRegisLogin>
  );
};
