...
const Header = ({ userInfor }) => {
  ...
  const handleLogout = () => {
    const confirmed = window.confirm('Bạn vẫn muốn đăng xuất phải không?');
    if (confirmed) {
  ...
  return (
    <div >
      <div className="w-full h-[70px] bg-gradient-to-r from-cyan-500 to-cyan-700 text-white flex justify-between px-20 items-center ">
        <p className="text-xl font-semibold">
          Chào mừng bạn đến với Juice Factory!
        </p>
        <div className=" font-semibold">
          {token ? (
            <div className="flex gap-10">
              <p>
                Xin chào, &nbsp;
                <span className="font-bold">{userInfor?.fullName}</span>
              </p>
      ...
      <div className="w-full h-[160px] flex items-center justify-between px-20 bg-white bg-gradient-to-r from-blue-400 to-blue-600 ">
        <img
          className=" w-40 h-20 bg-gradient-to-r from-cyan-500 to-cyan-700 pr-3 rounded-full"
          src={logo}
          alt=""
        />
        <SearchField />
        ...
        <div className="flex gap-3 items-center pr-10">
          <div className="relative w-10 inline-block ">
            <div onClick={() => setIsOpenCart(!isOpenCart)}>
              ...
              {token ? (
                <div className=" absolute -top-2 right-0 bg-gradient-to-r from-orange-300 to-orange-500 text-white rounded-full w-5 h-5 text-center leading-5 text-xs">
                  {totalItems || 0}
                </div>
              ):(
                ""
            ...
            <div
              className={`absolute z-30 min-w-[300px] mt-2.5 right-0 bg-white shadow-lg ${
                isOpenCart ? "visible" : "invisible"
              } cursor-pointer`}
            >
              <div>
                ...
                <div className=" font-semibold text-xs flex justify-around mb-5">
                  <Link
                    to="/cart"
                    className="uppercase w-[120px] border border-gray-300 border-solid py-2 px-5 rounded-full flex justify-center"
                  >
                    Giỏ hàng
                  </Link>
          ...
          {token ? (
            userInfor?.avatar ? (
              ...
              <PersonIcon fontSize="large" style={{ fill: "orange" }} />
            )
          ) : (
            // <PersonIcon fontSize="large" style={{ fill: "orange" }} />
            <img
